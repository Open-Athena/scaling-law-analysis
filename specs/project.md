...
3. **Keep specs in sync.** Spec → implementation is lossless (specs must be faithfully implemented), but implementation → spec is lossy (implementation may contain details not captured in specs). When a change affects a topic covered by a spec, update the other side to match (see [sync.md](sync.md)). Seek operator approval before making spec changes.
...
| `  common.py` | Shared generic utilities |
...
| `  data/` | IsoFLOP data pipeline |
| `    extract.py` | Fetches published IsoFLOP data from remote sources, standardizes to common schema |
| `    schema.py` | Pydantic schema definitions, outlier reason and QC stage enums |
| `    transform.py` | 8-step QC filter (deduplication, curvature checks, spline outlier detection), parabola fitting, unified CSV output |
| `    visualize.py` | IsoFLOP curve plots with outlier annotations |
| `    pipeline.py` | End-to-end runner: extract → transform → visualize |
...
| `data/` | Raw input data (IsoFLOP CSVs) |
| `docs/` | Supplementary documentation |
| `  images/` | Reference images |
| `  references/` | `references.yaml` — source of truth for citations |
| `  reproductions/` | Saved third-party reproduction results (e.g. ml-scalefit) |
