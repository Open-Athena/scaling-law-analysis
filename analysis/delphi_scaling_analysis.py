...
def _fit_power_law_to_minima(D: np.ndarray, L: np.ndarray) -> MinimaPowerLaw:
    ...
    # Bounds: x0 >= 0 (loss offset), x1 >= 0 (coefficient), -1 <= x2 <= 0 (exponent)
    x0_bounds = (0, np.inf)
    x1_bounds = (0, np.inf)
    x2_bounds = (-1.0, 0)
    lb = np.array([x0_bounds[0], x1_bounds[0], x2_bounds[0]])
    ub = np.array([x0_bounds[1], x1_bounds[1], x2_bounds[1]])

    popt, pcov, _infodict, mesg, ier = curve_fit(  # pyrefly: ignore
        model,
        D,
        L,
        p0=[L.min(), 1.0, -0.1],
    ...
    if ier not in (1, 2, 3, 4):
        raise RuntimeError(f"Power law fit failed (ier={ier}): {mesg}")
    if np.any(np.isinf(pcov)):
        raise RuntimeError(
            f"Power law fit did not converge: covariance is infinite. "
            f"params: x0={popt[0]:.4f}, x1={popt[1]:.4f}, x2={popt[2]:.4f}"
        )
    atol = 1e-6
    if np.any(popt - lb < atol) or np.any(ub - popt < atol):
        raise RuntimeError(
            f"Power law fit hit bounds: x0={popt[0]:.6f}, x1={popt[1]:.6f}, "
            f"x2={popt[2]:.6f} (bounds: lb={lb.tolist()}, ub={ub.tolist()})"
        )
    pred = model(D, *popt)
