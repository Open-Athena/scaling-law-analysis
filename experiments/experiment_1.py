...
def plot_isoflop_fits(
    ...
    for i, (C, fit) in enumerate(zip(compute_budgets, result.parabola_fits)):
        # Get sampled data
        N, D, L = isoflop_sample(C, 20, log_range)
        log_N = np.log10(N)
        log_L = np.log10(L)

        # Plot data points
        ax.scatter(log_N, log_L, c=[colors[i]], s=20, alpha=0.7, zorder=3)

        # Plot parabola fit
        log_N_fine = np.linspace(log_N.min(), log_N.max(), 100)
        log_L_fit = np.polyval(fit.coeffs, log_N_fine)
        ax.plot(log_N_fine, log_L_fit, c=colors[i], lw=1.5, alpha=0.8)
...
def create_figure(
    ...
    fig.suptitle(
        f"Experiment 1: Approach 2 Accuracy vs Grid Resolution\n"
...
def main():
    ...
    results = run_experiment(compute_budgets, log_ranges, n_points)
