...
I’ll start off by saying that I have no idea how to define exactly what neural scaling laws are now, which is probably not a great start for an article that is critical of a well-established method for finding them. I can at least say that a good definition likely lies somewhere between where they appear to be going: “economic outputs as a function of economic inputs” and where they started out: “validation cross-entropy loss as a function of parameter count”.

At that first extreme, you can imagine that “economic outputs” (e.g. human labor equivalents) are never going to be so predictable from “economic inputs” (e.g. hero runs at fractional GDP cost) that they warrant being called a “law”. At the other extreme, it’s much easier to understand how more direct relationships between capability like validation loss, eval performance, task time-horizon, etc. and resources like model size, data volume, FLOPs, etc. might be quantifiable with higher fidelity.

My point is that the evolution of neural scaling laws is no longer easy to characterize. Their implications have expanded along with the difficulties inherent to quantifying them, and both of these motivate a lot of my personal interest in this space. This is ultimately what brought me to looking carefully at how to approach scaling at Open Athena where we work on a number of problems in materials science, climate and weather, genomics, proteomics and language (via [Marin](https://marin.community/)). 

More specifically, our first attempt at developing scaling laws in this kind of multimodal scientific landscape started with DNA. It was not at all clear what kinds of problems might come up when lifting methods from language modeling to quantify these laws, but we found a couple and they eventually made their way into this project: [Problems with Chinchilla Approach 2](https://openathena.ai/scaling-law-analysis/). The rest of this post tries to give some intuition for what problems we found, what implications they have, and how they can be avoided in the future.
...
The math of Approach 2 works by first assuming parabolic approximations to isocontours of the loss surface defined by equal amounts of compute. Compute isn’t a parameter of the loss surface itself, but it is used to constrain the relationship between parameter count (N) and token count (D) through the approximation C=6ND. Again, our initial application of this approach outside of text was in DNA modeling, and the tiny vocabularies in those models end up breaking this assumption – I’ll conveniently ignore that for now and keep going. This constraint then gives the formula for an IsoFLOP curve as a univariate function of D (or N) alone:
...
I’m not entirely sure it was worth doing all this work. I was originally doing this to fit a very basic scaling law on some DNA data with Approach 2, and staring at enough wonky parabola fits ultimately got me thinking harder about how that method really works. If I had known that I’d eventually end up with an alternative that has so many advantages, I probably would have switched methods and called it a day. It just didn’t work out that way – the advantages were never clear at the start and they rolled out in a very nonlinear order.

Hopefully, there is something in between these simulations, proofs, the IsoFLOP data we gathered and curated, the reparameterization we proposed, and the literature we synthesized that will help nudge the space of scaling law modeling towards better standardization. In our paper, we gave more than 10 examples of analytical extensions to Chinchilla laws that have already been published. Many of these are very recent too, so I think it’s fair to say that this is a rapidly evolving space and that, much like the original Kaplan and Hoffman scaling laws, its potential for long-term impact is high. I think it's also fair to say that the space would benefit a lot from more efficient, cohesive strategies for fitting experimental data.
