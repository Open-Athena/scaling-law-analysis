...
realizations, yielding 9{,}216 fits per method and 36{,}864 fits in total.
...
mean error of 16.2\% and a maximum error on the $a$ exponent exceeding 290\%.
...
but its geometric mean error of 4.8\% is more than four times higher than VPNLS. Unlike
...
exceeds 700\%, driven by configurations with few points per curve where the
...
mean error of 1.1\%. As in the parameter recovery comparison
...
maximum error on the $a$ exponent reaches 73.6\%, compared to 34.2\% for
VPNLS.
...
accuracy (geometric mean of 1.1\% vs 1.1\%), but Approach~3's eight largest
errors range from 66\% to 74\%, whereas VPNLS's eight largest all fall
below 35\%. The detailed breakdown in
...
persists. Its geometric mean exponent error (4.8\%) is more than four times that of VPNLS
(1.1\%), and its worst-case error on the $a$ exponent exceeds 700\%.
Approach~3 without careful initialization fares even worse, with a geometric
mean of 16.2\% and worst-case errors approaching 300\%. With grid
initialization, Approach~3 matches VPNLS in typical accuracy but produces
sporadic large errors (max 73.6\% vs 34.2\% on $a$) that appear unpredictably
across IsoFLOP experiment design conditions. VPNLS offers the most reliable accuracy overall.
...
\begin{figure}[ht]
    ...
    points-per-curve setting (4, 8, 16, 32), over 256 noise realizations.
