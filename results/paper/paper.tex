...
informed by prior estimates or heuristics, and they will inevitably be wrong by
some amount.
...
values. None of these reliably solve the problem. The survey's own experiments
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{../article/method_comparison/method_comparison.png}
...
exhibits a similar pattern at somewhat lower magnitude. The full per-parameter
breakdown (Figure~\ref{fig:appendix_method_detail}) shows these instability
patterns in detail.
...
from scipy are not necessarily problematic. That said, VPNLS with Nelder-Mead
...
the fragile gradient tuning that complicates L-BFGS-B. All five loss surface
parameters $(E, A, B, \alpha, \beta)$ are recovered with machine precision, and
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=\textwidth]{../article/appendix/method_comparison_detailed.png}
